...
const Marks = () => {
  ...
  const setStudentMarksHandler = (enrollment, value) => {
    if (!value || value.trim() === '' || isNaN(parseFloat(value))) {
      toast.error(`Please enter valid marks for ${enrollment}`);
      return;
    }
    const headers = {
      "Content-Type": "application/json",
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          currentMarks[selected.subject] = parseFloat(value);
  ...
  useEffect(() => {
    if (selectedSemester) {
      const getSubjectData = async (e) => {
        ...
        toast.loading("Loading Subjects");
        axios
          .post(`${baseApiURL()}/subject/getSubject`, { semester: parseInt(search) }, { headers })
          .then((response) => {
            toast.dismiss();
            if (response.data.success) {
              setSubject(response.data.subject);
