      ...
      .then((response) => {
        ...
        if (response.data.success) {
          if (response.data.user.length === 0) {
            ...
            setStudents(response.data.user.map(student => ({ ...student, isPresent: false })));
            setShowTable(true);
          }
        } else {
      ...
      <div className="mt-8 w-full">
        ...
        <div className="flex justify-center space-x-4 mt-12">
          {showTable && students.length > 0 && (
            <>
              <button
                className="px-4 py-2 text-xl flex justify-center items-center text-white bg-green-500 hover:bg-green-600 rounded-md"
