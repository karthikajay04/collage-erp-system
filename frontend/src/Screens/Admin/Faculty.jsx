...
const Faculty = () => {
  ...
  const deleteFacultyHandler = (id) => {
    if (window.confirm("Are you sure you want to delete this faculty member?")) {
      ...
      axios
        .delete(`${baseApiURL()}/faculty/details/deleteDetails/${id}`, {
          headers: headers,
        })
        .then((response) => {
          toast.dismiss();
          if (response.data.success) {
            toast.success(response.data.message);
            viewFacultyHandler(); // Refresh the list
          } else {
            toast.error(response.data.message);
          }
        })
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-blue-300">
                  ...
                  <th className="py-2 px-4 border border-blue-700">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {faculty.sort((a, b) => {
                      ...
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                        ...
                        <td className="py-2 px-4 border border-blue-700 text-center">{`${item.firstName} ${item.middleName} ${item.lastName}`}</td>
